...
fun StashNavHost(
    ...
    NavHost(
        ...
        exitTransition = {
            if (targetState.destination.route == NowPlayingRoute::class.qualifiedName) {
                androidx.compose.animation.fadeOut(animationSpec = tween(SLIDE_DURATION_MS, delayMillis = SLIDE_DURATION_MS))
            } else {
                androidx.compose.animation.fadeOut()
            }
        },
        popEnterTransition = {
            if (initialState.destination.route == NowPlayingRoute::class.qualifiedName) {
                androidx.compose.animation.fadeIn(animationSpec = tween(1))
            } else {
        ...
        composable<NowPlayingRoute>(
            ...
            exitTransition = {
                if (targetState.destination.route == EqualizerRoute::class.qualifiedName || 
                    targetState.destination.route == ArtistDetailRoute::class.qualifiedName) {
                    androidx.compose.animation.fadeOut(animationSpec = tween(SLIDE_DURATION_MS))
                } else {
                    slideOutOfContainer(
            ...
            popEnterTransition = {
                if (initialState.destination.route == EqualizerRoute::class.qualifiedName || 
                    initialState.destination.route == ArtistDetailRoute::class.qualifiedName) {
                    androidx.compose.animation.fadeIn(animationSpec = tween(SLIDE_DURATION_MS))
                } else {
                    slideIntoContainer(
                        towards = AnimatedContentTransitionScope.SlideDirection.Up,
                        animationSpec = tween(SLIDE_DURATION_MS),
            ...
            NowPlayingScreen(
                ...
                onNavigateToEq = { navController.navigate(EqualizerRoute) },
                onNavigateToArtist = { artistName -> navController.navigate(ArtistDetailRoute(artistName)) }
            )
        }
    }
}
